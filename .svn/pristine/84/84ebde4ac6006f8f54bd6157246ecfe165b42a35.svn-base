...
abstract class Incident implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildIncidentVersion[] Collection to store aggregation of ChildIncidentVersion objects.
     */
    protected $collIncidentVersions;
    protected $collIncidentVersionsPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildIncidentVersion[]
     */
    protected $incidentVersionsScheduledForDeletion = null;
    ...
    public function setSeverity($v)
    {
        ...
        if ($this->severity !== $v) {
            $this->severity = $v;
            $this->modifiedColumns[IncidentTableMap::COL_SEVERITY] = true;
        }
    ...
    public function setVersion($v)
    {
        ...
        if ($this->version !== $v) {
            $this->version = $v;
            $this->modifiedColumns[IncidentTableMap::COL_VERSION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : IncidentTableMap::translateFieldName('Severity', TableMap::TYPE_PHPNAME, $indexType)];
            $this->severity = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : IncidentTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : IncidentTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->updated_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : IncidentTableMap::translateFieldName('Version', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 10; // 10 = IncidentTableMap::NUM_HYDRATE_COLUMNS.
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collIncidentVersions = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->incidentVersionsScheduledForDeletion !== null) {
                if (!$this->incidentVersionsScheduledForDeletion->isEmpty()) {
                    \IncidentVersionQuery::create()
                        ->filterByPrimaryKeys($this->incidentVersionsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->incidentVersionsScheduledForDeletion = null;
                }
            }

            if ($this->collIncidentVersions !== null) {
                foreach ($this->collIncidentVersions as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(IncidentTableMap::COL_SEVERITY)) {
        ...
        if ($this->isColumnModified(IncidentTableMap::COL_VERSION)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collIncidentVersions) {

                switch ($keyType) {
                    case TableMap::TYPE_CAMELNAME:
                        $key = 'incidentVersions';
                        break;
                    case TableMap::TYPE_FIELDNAME:
                        $key = 'incident_versions';
                        break;
                    default:
                        $key = 'IncidentVersions';
                }

                $result[$key] = $this->collIncidentVersions->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(IncidentTableMap::COL_SEVERITY)) {
            $criteria->add(IncidentTableMap::COL_SEVERITY, $this->severity);
        }
        if ($this->isColumnModified(IncidentTableMap::COL_CREATED_AT)) {
        ...
        if ($this->isColumnModified(IncidentTableMap::COL_VERSION)) {
            $criteria->add(IncidentTableMap::COL_VERSION, $this->version);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getIncidentVersions() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addIncidentVersion($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('IncidentVersion' == $relationName) {
            return $this->initIncidentVersions();
        }
    }
    ...
    /**
     * Clears out the collIncidentVersions collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addIncidentVersions()
     */
    public function clearIncidentVersions()
    {
        $this->collIncidentVersions = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collIncidentVersions collection loaded partially.
     */
    public function resetPartialIncidentVersions($v = true)
    {
        $this->collIncidentVersionsPartial = $v;
    }

    /**
     * Initializes the collIncidentVersions collection.
     *
     * By default this just sets the collIncidentVersions collection to an empty array (like clearcollIncidentVersions());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initIncidentVersions($overrideExisting = true)
    {
        if (null !== $this->collIncidentVersions && !$overrideExisting) {
            return;
        }

        $collectionClassName = IncidentVersionTableMap::getTableMap()->getCollectionClassName();

        $this->collIncidentVersions = new $collectionClassName;
        $this->collIncidentVersions->setModel('\IncidentVersion');
    }

    /**
     * Gets an array of ChildIncidentVersion objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildIncidentVersion[] List of ChildIncidentVersion objects
     * @throws PropelException
     */
    public function getIncidentVersions(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collIncidentVersionsPartial && !$this->isNew();
        if (null === $this->collIncidentVersions || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collIncidentVersions) {
                // return empty collection
                $this->initIncidentVersions();
            } else {
                $collIncidentVersions = ChildIncidentVersionQuery::create(null, $criteria)
                    ->filterByIncident($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collIncidentVersionsPartial && count($collIncidentVersions)) {
                        $this->initIncidentVersions(false);

                        foreach ($collIncidentVersions as $obj) {
                            if (false == $this->collIncidentVersions->contains($obj)) {
                                $this->collIncidentVersions->append($obj);
                            }
                        }

                        $this->collIncidentVersionsPartial = true;
                    }

                    return $collIncidentVersions;
                }

                if ($partial && $this->collIncidentVersions) {
                    foreach ($this->collIncidentVersions as $obj) {
                        if ($obj->isNew()) {
                            $collIncidentVersions[] = $obj;
                        }
                    }
                }

                $this->collIncidentVersions = $collIncidentVersions;
                $this->collIncidentVersionsPartial = false;
            }
        }

        return $this->collIncidentVersions;
    }

    /**
     * Sets a collection of ChildIncidentVersion objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $incidentVersions A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildIncident The current object (for fluent API support)
     */
    public function setIncidentVersions(Collection $incidentVersions, ConnectionInterface $con = null)
    {
        /** @var ChildIncidentVersion[] $incidentVersionsToDelete */
        $incidentVersionsToDelete = $this->getIncidentVersions(new Criteria(), $con)->diff($incidentVersions);


        //since at least one column in the foreign key is at the same time a PK
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->incidentVersionsScheduledForDeletion = clone $incidentVersionsToDelete;

        foreach ($incidentVersionsToDelete as $incidentVersionRemoved) {
            $incidentVersionRemoved->setIncident(null);
        }

        $this->collIncidentVersions = null;
        foreach ($incidentVersions as $incidentVersion) {
            $this->addIncidentVersion($incidentVersion);
        }

        $this->collIncidentVersions = $incidentVersions;
        $this->collIncidentVersionsPartial = false;
    ...
    /**
     * Returns the number of related IncidentVersion objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related IncidentVersion objects.
     * @throws PropelException
     */
    public function countIncidentVersions(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collIncidentVersionsPartial && !$this->isNew();
        if (null === $this->collIncidentVersions || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collIncidentVersions) {
            ...
            if ($partial && !$criteria) {
                return count($this->getIncidentVersions());
            }

            $query = ChildIncidentVersionQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collIncidentVersions);
    }

    /**
     * Method called to associate a ChildIncidentVersion object to this object
     * through the ChildIncidentVersion foreign key attribute.
     *
     * @param  ChildIncidentVersion $l ChildIncidentVersion
     * @return $this|\Incident The current object (for fluent API support)
     */
    public function addIncidentVersion(ChildIncidentVersion $l)
    {
        if ($this->collIncidentVersions === null) {
            $this->initIncidentVersions();
            $this->collIncidentVersionsPartial = true;
        ...
        if (!$this->collIncidentVersions->contains($l)) {
            $this->doAddIncidentVersion($l);

            if ($this->incidentVersionsScheduledForDeletion and $this->incidentVersionsScheduledForDeletion->contains($l)) {
                $this->incidentVersionsScheduledForDeletion->remove($this->incidentVersionsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildIncidentVersion $incidentVersion The ChildIncidentVersion object to add.
     */
    protected function doAddIncidentVersion(ChildIncidentVersion $incidentVersion)
    {
        $this->collIncidentVersions[]= $incidentVersion;
        $incidentVersion->setIncident($this);
    }

    /**
     * @param  ChildIncidentVersion $incidentVersion The ChildIncidentVersion object to remove.
     * @return $this|ChildIncident The current object (for fluent API support)
     */
    public function removeIncidentVersion(ChildIncidentVersion $incidentVersion)
    {
        if ($this->getIncidentVersions()->contains($incidentVersion)) {
            $pos = $this->collIncidentVersions->search($incidentVersion);
            $this->collIncidentVersions->remove($pos);
            if (null === $this->incidentVersionsScheduledForDeletion) {
                $this->incidentVersionsScheduledForDeletion = clone $this->collIncidentVersions;
                $this->incidentVersionsScheduledForDeletion->clear();
            }
            $this->incidentVersionsScheduledForDeletion[]= clone $incidentVersion;
            $incidentVersion->setIncident(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collIncidentVersions) {
                foreach ($this->collIncidentVersions as $o) {
                    $o->clearAllReferences($deep);
                }
            }
            if ($this->collCategories) {
    ...
    public function isVersioningNecessary($con = null)
    {
        ...
        if (ChildIncidentQuery::isVersioningEnabled() && ($this->isNew() || $this->isModified()) || $this->isDeleted()) {
            return true;
        }
     ...
     *
     * @return  ChildIncidentVersion A version object
     */
    public function addVersion($con = null)
    {
        $this->enforceVersion = false;

        $version = new ChildIncidentVersion();
        $version->setId($this->getId());
        $version->setTitle($this->getTitle());
        $version->setLocation($this->getLocation());
        $version->setLatitude($this->getLatitude());
        $version->setLongitude($this->getLongitude());
        $version->setActive($this->getActive());
        $version->setSeverity($this->getSeverity());
        $version->setCreatedAt($this->getCreatedAt());
        $version->setUpdatedAt($this->getUpdatedAt());
        $version->setVersion($this->getVersion());
        $version->setIncident($this);
        $version->save($con);
    ...
    public function toVersion($versionNumber, $con = null)
    {
        ...
        if (!$version) {
            throw new PropelException(sprintf('No ChildIncident object found with version %d', $version));
        }
     ...
     * Sets the properties of the current object to the value they had at a specific version
     *
     * @param ChildIncidentVersion $version The version object to use
     * @param ConnectionInterface   $con the connection to use
    ...
    {
        $v = ChildIncidentVersionQuery::create()
            ->filterByIncident($this)
            ->orderByVersion('desc')
            ->findOne($con);
        if (!$v) {
    ...
    {
        return ChildIncidentVersionQuery::create()
            ->filterByIncident($this)
     ...
     * @param   ConnectionInterface $con the connection to use
     *
     * @return  ObjectCollection|ChildIncidentVersion[] A list of ChildIncidentVersion objects
     */
    public function getAllVersions($con = null)
    {
        $criteria = new Criteria();
        $criteria->addAscendingOrderByColumn(IncidentVersionTableMap::COL_VERSION);

        return $this->getIncidentVersions($criteria, $con);
    }
     ...
     * @return PropelCollection|\IncidentVersion[] List of \IncidentVersion objects
     */
    public function getLastVersions($number = 10, $criteria = null, $con = null)
    {
        $criteria = ChildIncidentVersionQuery::create(null, $criteria);
        $criteria->addDescendingOrderByColumn(IncidentVersionTableMap::COL_VERSION);
