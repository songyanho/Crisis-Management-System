 ...
 * @method     ChildIncidentQuery orderByVersion($order = Criteria::ASC) Order by the version column
 *
 ...
 * @method     ChildIncidentQuery groupByVersion() Group by the version column
 *
 ...
 *
 * @method     ChildIncidentQuery leftJoinIncidentVersion($relationAlias = null) Adds a LEFT JOIN clause to the query using the IncidentVersion relation
 * @method     ChildIncidentQuery rightJoinIncidentVersion($relationAlias = null) Adds a RIGHT JOIN clause to the query using the IncidentVersion relation
 * @method     ChildIncidentQuery innerJoinIncidentVersion($relationAlias = null) Adds a INNER JOIN clause to the query using the IncidentVersion relation
 *
 * @method     ChildIncidentQuery joinWithIncidentVersion($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the IncidentVersion relation
 *
 * @method     ChildIncidentQuery leftJoinWithIncidentVersion() Adds a LEFT JOIN clause and with to the query using the IncidentVersion relation
 * @method     ChildIncidentQuery rightJoinWithIncidentVersion() Adds a RIGHT JOIN clause and with to the query using the IncidentVersion relation
 * @method     ChildIncidentQuery innerJoinWithIncidentVersion() Adds a INNER JOIN clause and with to the query using the IncidentVersion relation
 *
 * @method     \IncidentCategoryQuery|\IncidentReporterQuery|\IncidentResourceQuery|\IncidentResourceRecordQuery|\IncidentVersionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildIncident findOneByVersion(int $version) Return the first ChildIncident filtered by the version column *
 ...
 * @method     ChildIncident requireOneByVersion(int $version) Return the first ChildIncident filtered by the version column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildIncident[]|ObjectCollection findByVersion(int $version) Return ChildIncident objects filtered by the version column
...
protected $entityNotFoundExceptionClass = '\\Propel\\Runtime\\Exception\\EntityNotFoundException';
    ...
    {
        $sql = 'SELECT id, title, location, latitude, longitude, active, severity, created_at, updated_at, version FROM incident WHERE id = :p0';
        try {
    ...
    public function filterByVersion($version = null, $comparison = null)
    {
        if (is_array($version)) {
            ...
            if (isset($version['min'])) {
                $this->addUsingAlias(IncidentTableMap::COL_VERSION, $version['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($version['max'])) {
                $this->addUsingAlias(IncidentTableMap::COL_VERSION, $version['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(IncidentTableMap::COL_VERSION, $version, $comparison);
    }
    ...
    /**
     * Filter the query by a related \IncidentVersion object
     *
     * @param \IncidentVersion|ObjectCollection $incidentVersion the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildIncidentQuery The current query, for fluid interface
     */
    public function filterByIncidentVersion($incidentVersion, $comparison = null)
    {
        if ($incidentVersion instanceof \IncidentVersion) {
            return $this
                ->addUsingAlias(IncidentTableMap::COL_ID, $incidentVersion->getId(), $comparison);
        } elseif ($incidentVersion instanceof ObjectCollection) {
            return $this
                ->useIncidentVersionQuery()
                ->filterByPrimaryKeys($incidentVersion->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByIncidentVersion() only accepts arguments of type \IncidentVersion or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the IncidentVersion relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return $this|ChildIncidentQuery The current query, for fluid interface
     */
    public function joinIncidentVersion($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('IncidentVersion');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'IncidentVersion');
        }
    ...
    /**
     * Use the IncidentVersion relation IncidentVersion object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \IncidentVersionQuery A secondary query class using the current class as primary query
     */
    public function useIncidentVersionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinIncidentVersion($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'IncidentVersion', '\IncidentVersionQuery');
    }
